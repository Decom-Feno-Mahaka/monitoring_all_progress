...
import { ProjectCategory, ProjectStatus, HealthStatus, ProjectVisibility, CATEGORY_CONFIG } from '@/lib/types';
...
export default function NewProjectPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Upload files & attach evidences if any
      if (evidences.length > 0) {
        ...
        const actRes = await activitiesApi.create(projectId, {
          type: 'EVIDENCE_ADDED',
          title: 'Initial Evidence',
          description: `${uploaded.length} evidence item(s) added at project creation.`,
        });
        ...
        if (evidencePayload.length > 0) {
          await activitiesApi.addEvidence(projectId, activityId, evidencePayload);
        }
      }
    ...
    <div className="max-w-2xl">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-[rgb(var(--text-muted))]">Create a new project to track</p>
        </div>
      </div>
      ...
      >
        ...
        <div className="card p-5 space-y-4">
          ...
          {newEvidType === 'DOCUMENT' && (
            <label className="flex flex-col items-center gap-2 p-5 rounded-xl border-2 border-dashed border-[rgb(var(--border))] hover:border-[#00B9D9]/50 transition-colors cursor-pointer text-center">
              <Upload size={22} className="text-[rgb(var(--text-muted))]" />
              ...
              <input
                ...
                multiple
                onChange={e => handleFileEvidence(e, 'DOCUMENT')}
              />
            </label>
          )}
          ...
          {newEvidType === 'IMAGE' && (
            <label className="flex flex-col items-center gap-2 p-5 rounded-xl border-2 border-dashed border-[rgb(var(--border))] hover:border-[#00B9D9]/50 transition-colors cursor-pointer text-center">
              <ImageIcon size={22} className="text-[rgb(var(--text-muted))]" />
              <span className="text-xs text-[rgb(var(--text-secondary))]">Klik untuk pilih foto <span className="text-[rgb(var(--text-muted))]">(JPG, PNG, GIF, WebP)</span></span>
              <input
                type="file"
                className="hidden"
                accept="image/*"
                multiple
                onChange={e => handleFileEvidence(e, 'IMAGE')}
              />
            </label>
          )}
              ...
              {evidences.map(ev => (
                ...
                >
                  ...
                  {ev.type === 'IMAGE' && ev.preview ? (
                    ...
                    <div
                      className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-[#00B9D9]"
                      style={{ background: 'rgb(0 185 217 / 0.1)' }}
                    >
                  ...
                  <button
                    ...
                    className="text-[rgb(var(--text-muted))] hover:text-red-400 transition-colors flex-shrink-0"
                  >
                    <X size={14} />
                  </button>
                </div>
          ...
          >
            {saving ? (
              <span className="flex items-center gap-2">
                ...
                {evidences.length > 0 ? 'Uploading evidence...' : 'Creating...'}
              </span>
            ) : (
              <><Save size={16} /> Create Project {evidences.length > 0 && `(+${evidences.length} evidence)`}</>
            )}
          </motion.button>
        </div>
      </motion.form>
